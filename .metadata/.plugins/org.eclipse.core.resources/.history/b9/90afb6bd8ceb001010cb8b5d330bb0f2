...
public class ChangePassword1FA2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangePassword1FA2() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		...
		if(userHash!=null){
			...
			System.out.println("<pib><ChangePassword1FA2>userHash from Client is null!");
		}
		
		
		if(validCookieBool){
			String userName=ssocs.getLoginName(userHash);
			System.out.println("<fyp2><ChangePassword1FA2>userName= "+userName);
			...
			if(b){
				RequestDispatcher dispatcher = request.getRequestDispatcher("/Password/changePassword1FA2Success.jsp");
				if (dispatcher != null) dispatcher.forward(request, response);
			}
			else
			{
				RequestDispatcher dispatcher = request.getRequestDispatcher("/Password/changePassword1FA2Fail.jsp");
				if (dispatcher != null) dispatcher.forward(request, response);
			}
