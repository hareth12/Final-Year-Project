...
import chinese.data.PayPayee2FAData;
import chinese.service.RBKClientService;
import chinese.service.SSOClientService;


public class FundTransfer2FA1Action2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FundTransfer2FA1Action2() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(validCookieBool){
			...
			String payeeIndexHash = request.getParameter("toAccount");
			String amountString = request.getParameter("amount");
			double amount = Double.parseDouble(amountString);
			
			List<AccountLinkDataStructure> alds = rbkcs.getPayeeList(userName);
			long toAccount = rbkcs.getToAccountNumber(payeeIndexHash);
			String toAccountString = Long.toString(toAccount);
			
			PayPayee2FAData x = rbkcs.payPayee2FA1(payeeIndexHash, amount);
			...
			request.setAttribute("indexHash", payeeIndexHash);	
			request.setAttribute("amount", amountString);	
			request.setAttribute("preHash", x.getPreHash());
			
			
			System.out.println("<fyp2><FundTransfer2FA1Action2> userName    = "+userName);
			System.out.println("<fyp2><FundTransfer2FA1Action2> fromAccount = "+fromAccountString);
			System.out.println("<fyp2><FundTransfer2FA1Action2> toAccount = "+toAccountString);
			System.out.println("<fyp2><FundTransfer2FA1Action2> payeeIndexHash = "+payeeIndexHash);
			System.out.println("<fyp2><FundTransfer2FA1Action2> amount = "+amount);
			
			RequestDispatcher dispatcher = request.getRequestDispatcher("/fundTransfer/fundTransfer2FA1Action2.jsp");
